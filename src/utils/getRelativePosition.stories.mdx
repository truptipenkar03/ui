...
Many times a single element needs to be positioned with respect to another. A very common case is a tooltip. Using the
`getRelativePosition` utility, for a given anchor element (button), floater element (tooltip) and position we can
calculate the `top` and `left` position values that need to be applied to the `floater` in order to position it
